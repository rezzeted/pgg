"""PGG MCP server — agent tooling over the PggServe RPC.

Stdio transport. Started as the single MCP server ``pgg`` via
``python3 -m tools.pgg_mcp.launch`` (venv) then ``python -m tools.pgg_mcp``.

Thin proxy to the PggServe RPC (TCP + line-delimited JSON on
127.0.0.1:9878, see ``docs/pgg/serve_rpc.md``). The same Python session
auto-starts ``PggServe`` on every OS; if the binary is missing the
tools return ``kind=need_build`` with configure/build argv. Every response
keeps the RPC envelope (``{"ok": true, "data": ...}`` /
``{"ok": false, "error"}``). Slot identity is the canonical ``.pgg`` path.
"""
...
_INSTRUCTIONS = """PGG MCP — итеративный цикл отладки .pgg-графов через PggServe.

Типовой цикл «правка → картинка + числа»:

1. ``pgg_status`` — демон жив (поднимается автоматически при первом вызове).
   Если ``error.kind=need_build`` — собрать PggServe командами из ответа
...
2. ``pgg_load`` (path до .pgg или source целиком) — статическая проверка без
   ...
   Слот = канонический путь файла. Два агента на двух файлах — два слота;
   на одном файле — один слот и общий кэш.
3. ``pgg_render`` (node или view) — PNG кадра + статы + ``render_state``.
   Незаданные args сбрасываются в дефолты (липкие только orbit/distance).
   Именованный вид — ``view`` из ``<stem>.views.json``. ``chrome`` и
   ``frame=window`` принимаются и игнорируются: кадр всегда FBO превью.
   ``size`` — размер FBO. Перед grep по спеке: ``pgg_docs("clip")``.
   Если в этом ходе не было ``pgg_load`` — передайте ``file=`` (слот).
4. ``pgg_probe`` (spec вида "house:schema" / "house:bbox[group=stone]") —
   инспектор-записи; зазор — ``pgg_measure``. Тоже ``file=``, если load
   был не в этом ходе.
5. Правка файла на диске → сразу ``pgg_render``/``pgg_probe``: F4 — сервер
   сам перечитывает файл и его импорты по mtime (ответ содержит reloaded=true),
...
@mcp.tool()
def pgg_status() -> dict:
    """Живость PggServe и слотов по файлам.

    При живом RPC — data: {serve:"running", viewer:"running", binary?,
    rpc:{host,port}, file, slots:[{file,cache,last_node}], params, cache,
    preview, profile, uptime_s}. Если бинаря нет — ok=false,
    error.kind=need_build (configure/build/debug_build argv, cwd, expected,
    hint). Пример: pgg_status().
    """
...
def pgg_load(path: Optional[str] = None, source: Optional[str] = None,
    ...
    """Загрузить .pgg-файл в слот (ровно один из path/source обязателен).

    path — путь до .pgg (относительно cwd демона = корня репо, или абсолютный);
    ...
    has_errors, ms, path, session:{file}}.
...
def pgg_params(params: dict[str, Any], file: Optional[str] = None) -> dict:
    ...
    в поле unknown. file — слот (канонический путь); без него — последний load
    этого MCP-процесса. Ответ data: {params:{...текущие...}, unknown:[...],
    session:{file}}.
    Пример: pgg_params({"stories": 2, "seed": 42}).
    """
    args = dict(params or {})
    return _call("params", _with_file(args, file))
...
def pgg_render(node: Optional[str] = None, out: Optional[str] = None, orbit: Optional[list[float]] = None,
               ...
               png: Optional[bool] = None, save_baseline: Optional[bool] = None,
    ...
    file — слот; обязателен, если в этом ходе не было pgg_load (два чата через
    один MCP иначе попадут не в тот файл). Кадр всегда FBO превью: chrome и
    frame=window принимаются и игнорируются. size — размер FBO (кламп к 4096,
    size_clamped=true только тогда).
    ...
    RPC-render **stateless**: незаданный wire/ortho/target/highlight/shading/colors/zoom
    сбрасывается в дефолт (wire=false, ortho=off, target="", zoom=1).
    ...
    target_bbox?), render_state:{wire,chrome,ortho,target,zoom,fit}, compare?, session:{file}.
    Пример: pgg_render(view="front", wire=False, chrome="off").
    """
    return _call("render", _with_file({"node": node, "out": out, "orbit": orbit,
                            ...
                            "save_baseline": save_baseline}, file))
...
def pgg_reference(image: str, node: str, ortho: Optional[str] = None,
                  ...
                  file: Optional[str] = None) -> dict:
    """Модель рядом с референсом: side-by-side PNG + силуэтные метрики.
    ...
    size — размер FBO модели в пикселях (кламп к kMaxTarget 4096,
    size_clamped=true в ответе). file — слот.
    ...
    cache:{hits,misses}, reloaded, load_diagnostics?, session:{file}}.
    Ошибки: image не читается — invalid_input; без GPU —
    no_gpu; прогон — run_failed/run_errors как у render.
    Пример: pgg_reference(image="tmp/reference.png", node="house", ortho="front").
    """
    return _call("reference", _with_file({"image": image, "node": node, "ortho": ortho,
                               "orbit": orbit, "zoom": zoom, "size": size}, file))
...
@mcp.tool()
def pgg_export(node: str, obj_path: str, file: Optional[str] = None) -> dict:
    ...
    with mesh_from_sdf»). file — слот.
    Ответ data: {path, realized, stats:{...},
    cache:{hits,misses}, reloaded, load_diagnostics?, session:{file}} (reloaded — авто-reload
    по mtime, F4).
    Пример: pgg_export(node="house", obj_path="tmp/house.obj").
    """
    return _call("export", _with_file({"node": node, "obj_path": obj_path}, file))


@mcp.tool()
def pgg_diff(update: Optional[bool] = None, file: Optional[str] = None) -> dict:
    """Diff outputs текущего слота против снимка fingerprints (C2, fp-level).

    Сервер хранит снимок структурных фингерпринтов outputs **на слот**. Первый
    вызов (или вызов после явного pgg_load) снимок СОЗДАЁТ и отвечает
    baseline_created=true (сравнения нет). Дальнейшие вызовы прогоняют outputs
    текущего файла и сравнивают: outputs:[{name, status: identical|changed|
    added|removed|skipped, fingerprint_prev, fingerprint_now}], identical —
    сводный вердикт (skipped — sdf/field без структурного fp, на вердикт не
    влияет; ΔP не считается — это fp-level diff, таблица изменений —
    `PggTool diff` в CLI). Правка файла на диске подхватывается авто-reload'ом
    ВНУТРИ diff (F4, reloaded=true) — снимок она НЕ сбрасывает. Снимок
    обновляется только update=true (snapshot_updated=true в ответе) или
    явным pgg_load — он снимок сбрасывает (pgg_load(..., snapshot=True)
    записывает свежий baseline сразу, иначе его запишет первый pgg_diff с
    baseline_created=true).
    file — слот.
    Ответ data: {baseline_created?|identical, outputs, snapshot_updated?,
    reloaded, load_diagnostics?, ms, cache:{hits,misses}, session:{file}}.
    ...
    """
    return _call("diff", _with_file({"update": update}, file))
...
def pgg_docs(symbol: str, file: Optional[str] = None) -> dict:
    ...
    file — слот для поиска def; без файла ищутся только builtin'ы.
    ...
    """
    return _call("docs", _with_file({"symbol": symbol}, file))


@mcp.tool()
def pgg_views(file: Optional[str] = None) -> dict:
...
def pgg_contact_sheet(views: Any = "*", cols: Optional[int] = None,
    ...
    Подписи — имя вида. PNG → tmp/pgg_rpc_shots/contact_sheet.png. file — слот.
    """
    try:
    ...
    stats: list[Any] = []
    for name in names:
        resp = pgg_render(view=name, png=True, size=size, chrome="off", file=file)
        if not resp.get("ok"):
            return resp
