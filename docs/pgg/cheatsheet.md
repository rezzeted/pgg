...
`sin`/`cos`/`atan2` — **радианы**; градусы → `radians()`. `mix`/`clamp`/`smoothstep`, `vec3()`, касты. `@P`/`@N`/`@index`/`@iteration`/`@tint`/`@Cd`; маска группы — `ingroup("brick")`; `mark(g, name, where)` задаёт членство заново, дописать в группу — `mark(..., mode = add)`.
...
_, mx = bbox(g)                                   # `_` отбрасывает выход
lid = transform(cap, translate = (0, mx.y, 0))     # swizzle .x/.xz; (a, b, c) из выражений — vec3
m2 = mark(m, "deck", where = @P.y > y0, domain = faces, mode = add)
landed = set_position(raycast(pts, target = terrain), pos = @hit_pos, where = @hit)
yard = delete(pts, where = !inside_polygon(outline))
```
Вызов можно переносить внутри `()`/`[]`; дефолты параметров — константные выражения (`= -1`, `= vec3(0, 0.5, -1)`, `= []`). `delete(..., domain = faces)` удаляет и осиротевшие точки.
...
- `plan_rect`: сторону выбирать `ap.rect_side(side = front)` (не литерал 0..3), начало/нормаль/yaw ребра — `ap.edge_frame(plan, i)`, контракт — шапка `lib/arch/plan`. Локальный +X фасада = касательная ребра (фронт +X, тыл −X, лево +Z, право −Z): симметричный паттерн это переживает, слот на конкретной оси (дверь на +wing_x) — через `tangent` из `edge_frame`. `edges_of_polygon` идёт CCW: выступ терминала в локальный +X окажется ВНУТРИ — для выступающих терминалов (карнизы) рёбра брать из `band_edges` (ход против +X фасада); симметричным профилям (`rail_run`) направление безразлично.
...
- Ребро контура `@pitch = 90` (фронтон щипца) даёт **вертикальную** панель и eave-плоскость своего островка: перед `tile_anchors`/покрытием убрать и панель (`delete(sl, where = @island_id == g, domain = faces)`), и её eave-строку, иначе фронтонная стена зарастает черепицей.
...
`pgg_docs("clip")` · `pgg_render(node="house", view="front", wire=False, chrome="off", file=…)` · `pgg_render(..., compare="prev"|"baseline", png=False)` · `pgg_contact_sheet(views="*")` · `pgg_measure(node="house", a="group:stone", b="group:brick", axis="x")` · `pgg_probe("house:bbox[group=stone]")` · `pgg_probe(specs=["roof:hist[attr=@island_id]", "roof:count"])` (разбивка по элементам) · `pgg_reference(image=..., node="hotel")` · `pgg_status()` → `kind=need_build` → собрать PggServe из `configure`/`build` и retry
