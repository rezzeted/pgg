...
- `delete(m, where = …, domain = faces)` **оставляет точки** удалённых граней; face-атрибут на них читается как 0 → `max_of(@eave_yaw, on = m)` у отрицательного азимута даёт 0, `min_of(dot(@P, U))` — экстент чужих панелей. Агрегат по своим граням: `set(m, "live", 1.0, domain = faces)` + `where = @live > 0.5` (идиома `anchors_of_panel` в `lib/arch/roof`).
- `mark(g, name, where)` **присваивает** членство (где `where = false` — снимает), а `parts.to_group(g, src, dst)` в уже существующую в меше группу снимает её с остальных граней — терминал собирать сразу в нужных группах, не перекрашивать в занятую.
- Отрицательный литерал в дефолте сигнатуры (`side: int = -1`) — E100; в аргументах вызова `-1` можно. Флаг стороны — `right: bool = false`.
- `roof_wavefront(overhang > 0)` на контуре с двумя выступами (ризалиты на соседних фасадах, 12+ вершин) даёт вершину за ~130 км (bbox крыши взрывается; Г/Т/один выступ — норма): свес — оффсетом контура в файле здания, `overhang = 0` (`resources/Mansion/mansion.pgg`).
